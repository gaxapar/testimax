...
class QuizDAO:
    ...
    async def get_random_quiz(self) -> Quiz | None:
        statement = select(Quiz).order_by(func.random()).limit(1)
    ...
    async def get_quizzes_page(self, page: int) -> Sequence[Quiz]:
        statement = (
            select(Quiz).order_by(Quiz.usages.desc(), Quiz.id.asc()).offset((page - 1) * PAGE_SIZE).limit(PAGE_SIZE)
        )
    ...
    async def get_quizzes_pages_count(self) -> int:
        statement = select(func.count()).select_from(Quiz)
